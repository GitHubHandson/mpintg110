 ...
 */
@Path("/marketplaceintegration/auth/{type}")
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class MarketplaceIntegrationAuthResource {
	private static final Logger LOG = LoggerFactory.getLogger(MarketplaceIntegrationAuthResource.class);
	...
	public void initialize() {
		LOG.info("MarketplaceIntegrationAuthResource initialized: " + new Date().toString());
	}
	...
	public DataHubResponse process(@PathParam("type") final String type, final String json) {
		LOG.debug("Received marketplaceintegration request, type: auth/" + type + " json: " + json);

		ResultData result = null;
		DataHubResponse datahubResponse = new DataHubResponse();

		result = marketplaceIntegrationFacade.process("auth/" + type, json);
