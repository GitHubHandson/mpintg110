 ...
 */
@Path("/marketplaceintegration/trade/{type}")
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class MarketplaceIntegrationTradeResource {
	private static final Logger LOG = LoggerFactory.getLogger(MarketplaceIntegrationTradeResource.class);
	...
	public void initialize() {
		LOG.info("MarketplaceIntegrationTradeResource initialized: " + new Date().toString());
	}
	...
	public DataHubResponse process(@PathParam("type") final String type, final String json) throws Exception {
		LOG.info("Received marketplaceintegration request, type: trade/" + type + " json: " + json);
		ResultData result = null;
		DataHubResponse datahubResponse = new DataHubResponse();

		result = marketplaceIntegrationFacade.process("trade/" + type, json);
