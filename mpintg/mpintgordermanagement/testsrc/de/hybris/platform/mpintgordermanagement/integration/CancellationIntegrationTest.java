/*package de.hybris.platform.mpintgordermanagement.integration;
...
import de.hybris.platform.mpintgordermanagement.actions.order.SourceOrderAction;
...
public class CancellationIntegrationTest extends ServicelayerTransactionalTest
{
	...
	public void setup() throws IOException, ImpExException
	{
		...
		importCsv("/mpintgordermanagement/test/impex/integration-test-data-default.impex", WarehousingTestConstants.ENCODING);
		importCsv("/mpintgordermanagement/test/impex/integration-test-data-simple.impex", WarehousingTestConstants.ENCODING);
...
final OrderProcessModel process = getBusinessProcessService().<OrderProcessModel> createProcess(
		order.getCode() + "_ordermanagement", MpintgordermanagementConstants.ORDER_PROCESS_NAME);
 ...
 * @Test
 * 
 * @Ignore public void createShippedEntryAfterCancellation() {
 * 
 * //Given productInfo.put(PRODUCT_1, new Long(3));
 * 
 * 
 * final OrderModel order = OrderBuilder.aSourcingOrder().build(buildAddress(), currency, productService, productInfo);
 * 
 ...
 * 
 * cancellationEntryInfo.put(order.getEntries().get(0), 1L); final Collection<CancellationEntry> cancellationEntry =
 * CancellationEntryBuilder.aCancellation() .build(cancellationEntryInfo); orderCancellationService.cancelOrder(order,
 * cancellationEntry);
 * 
 ...
 * 
 * //when consignmentResult.stream().forEach(result -> shippingService.createShippedEntries(result));
 * 
 * consignmentResult.stream().forEach(result -> shippedResult.addAll(shippingService.createShippedEntries(result)));
 * 
 * // Then assertTrue(shippedResult.size() == 1); assertTrue(shippedResult.stream().allMatch(result ->
 ...
 * 
 * final UserModel user = userService.getCurrentUser(); address.setOwner(user);
 * 
 * return address; }
 * 
