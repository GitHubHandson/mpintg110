 ...
 */
@Path("/marketplaceintegration/logistics/{type}")
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class MarketplaceIntegrationLogisticsResource {
	private static final Logger LOG = LoggerFactory.getLogger(MarketplaceIntegrationLogisticsResource.class);
	...
	public void initialize() {
		LOG.info("MarketplaceIntegrationLogisticsResource initialized: " + new Date().toString());
	}
	...
	public DataHubResponse process(@PathParam("type") final String type, final String json) {
		LOG.debug("Received marketplaceintegration request, type: logistics/" + type + " json: " + json);

		ResultData result = null;
		DataHubResponse datahubResponse = new DataHubResponse();
		result = marketplaceIntegrationFacade.process("logistics/" + type, json);
