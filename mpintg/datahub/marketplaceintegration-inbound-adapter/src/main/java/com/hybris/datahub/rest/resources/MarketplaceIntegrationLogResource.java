 ...
 */
@Path("/marketplaceintegration/log/{type}")
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class MarketplaceIntegrationLogResource {
	...
	public void initialize() {
		LOG.info("MarketplaceIntegrationLogResource initialized: " + new Date().toString());
	}
	...
	public DataHubResponse process(@PathParam("type") final String type, final String json) {
		LOG.debug("Received marketplaceintegration request, type: log/" + type + " json: " + json);

		ResultData result = null;
		DataHubResponse datahubResponse = new DataHubResponse();

		result = marketplaceIntegrationFacade.process("log/" + type, json);
