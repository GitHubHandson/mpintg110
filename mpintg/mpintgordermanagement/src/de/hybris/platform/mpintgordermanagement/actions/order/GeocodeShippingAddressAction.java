...
/**
 * Used to get an address from order.deliveryAddress and find the closest store populating back the delivery
 * latitude/longitude.
 */
	...
	public void executeAction(final OrderProcessModel orderProcessModel)
	{
		...
		try
		{
			if (!(order.getDeliveryMode() instanceof PickUpDeliveryModeModel))
			{
				...
				final GPS gps = getGeoWebServiceWrapper().geocodeAddress(getAddressConverter().convert(order.getDeliveryAddress()));
